...
# ***************************
    ...
    def wait_for_load(self):           
        ...
        if self.next_player<>None:
            if self.next_player.get_play_state()=='load_failed':
                ...
                if self.previous_player == None  and self.next_player.get_play_state()=='loaded':
    ...
    def close_and_load(self):
        ...
        if self.previous_player<>None and self.previous_player.get_play_state()=='pause_at_end':
    ...
    def close_current_and_next(self):
        if self.current_player<>None and self.current_player.get_play_state()=='pause_at_end':
            if self.trace: print 'artshow/what_next - closing_current from terminate'
            self.current_player.close(None)
        if self.next_player<>None and self.next_player.get_play_state() not in ('unloaded','load_failed'):
    ...
    def wait_for_end(self):
        ...
        ok_to_end=0
        if self.current_player==None or self.current_player.get_play_state()=='closed':
            self.current_player=None
            ok_to_end+=1
        if self.next_player==None or self.next_player.get_play_state() in ('unloaded','load_failed'):
            self.next_player=None
            ok_to_end+=1
        if ok_to_end<>2:
